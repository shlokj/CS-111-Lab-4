...
void write_block_group_descriptor_table(int fd) {
	...
	block_group_descriptor.bg_used_dirs_count = 2; // at the root directory, both . and .. are the same (root). that is one, and another is the lost+found directory
...
void write_block_bitmap(int fd) {
	...
	u8 bitmap[NUM_BLOCKS]; // even though we have only 1024 blocks and therefore 1024 bits, if we set the 1025th bit onwards to 1, we get the result we need
	...
	for (int i = 0; i < 1024; i++) {
		for (int j = 0; j < 8; j++) {
			u16 idx = (i * 8) + j + 1; // +1 since we don't want to track the boot block in the bitmap
			u8 is_used = idx <= LAST_BLOCK || idx >= NUM_BLOCKS; // evaluates to 0 or 1. for ints, idx < LAST_BLOCK is the same as idx <= LAST_BLOCK - 1, which is what we're aiming at
			bitmap[i] = (is_used << j) | bitmap[i];
		}
	}
...
void write_inode_bitmap(int fd) {
	...
	u8 bitmap[1024]; // even though we have only 1024 blocks and therefore 1024 bits, if we set the 1025th bit onwards to 1, we get the result we need
	...
	for (int i = 0; i < 1024; i++) {
		for (int j = 0; j < 8; j++) {
			u16 idx = (i * 8) + j + 1;
			u8 is_used = idx <= LAST_INO || idx > NUM_INODES; // since inodes are 1-indexed
			bitmap[i] = (is_used << j) | bitmap[i];
		}
	}
...
void write_hello_world_file_block(int fd) {
	...
	if (write(fd, hello_world_text, 12) != 12) {
		errno_exit("write");
	}
}
